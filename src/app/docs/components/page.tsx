...
export default function ComponentsIndexPage() {
  ...
  return (
    <div className="max-w-3xl">
      <p className="text-sm font-medium text-link">Components</p>
      <h1 className="mt-2 text-3xl font-semibold tracking-tight text-foreground">All components</h1>
      <p className="mt-4 max-w-xl text-base leading-7 text-muted-foreground">
        {groups.reduce((n, g) => n + g.items.length, 0)} patterns, grouped by where you would reach
        for them.
      </p>
            ...
            <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
              {group.items.map((item) => (
                ...
                >
                  ...
                  <p className="font-mono text-xs text-muted-foreground/80">{item.source}</p>
                </Link>
              ))}
            </div>
          </section>
